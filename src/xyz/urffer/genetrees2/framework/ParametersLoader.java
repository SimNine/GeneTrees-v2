...
//	private static String paramFilePath = GeneTrees.self.getClass().getResource("/xyz/urffer/genetrees2/framework/treeConfig.yaml").getFile();
	private static String paramFilePath = "treeConfig.yaml";
	
	private static Parameters params = null;// = new Parameters();
	
	public static Parameters getParams() {
		return params;
	}
	
	private static void generateYamlIfNotExist() {
		...
		if (!file.exists()) {
			...
			try {
				mapper.writeValue(file, new Parameters());
	...
	public static void loadYaml() {
		generateYamlIfNotExist();
		
		File file = new File(paramFilePath);
		
		ObjectMapper om = new ObjectMapper(new YAMLFactory());
		try {
			params = om.readValue(file, Parameters.class);
		} catch (IOException e) {
			e.printStackTrace();
		}
		
		System.out.println(params.envGroundLevel);
	}
